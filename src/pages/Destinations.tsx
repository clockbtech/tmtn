...
const Destinations = () => {
  ...
  const filteredAndSortedDestinations = useMemo(() => {
    let filtered = destinations.filter(destination => {
      const matchesSearch = t(destination.nameKey).toLowerCase().includes(searchTerm.toLowerCase()) ||
                           t(destination.descKey).toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const currentDestinations = filteredAndSortedDestinations.slice(
  ...
  return (
    <div className="min-h-screen bg-white font-poppins">
      ...
      <section 
        ...
        style={{
          backgroundImage: 'url(https://images.unsplash.com/photo-1426604966848-d7adac402bff?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&q=80)'
        }}
      >
        <div className="absolute inset-0 bg-gradient-to-r from-nepal-primary/80 to-blue-600/80"></div>
      ...
      <section className="py-8 bg-gray-50 border-b">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Filter Summary */}
          {(selectedRegions.length > 0 || searchTerm) && (
            <div className="mt-4 flex flex-wrap gap-2 items-center">
              <span className="text-sm text-gray-600">Active filters:</span>
              {searchTerm && (
                <span className="bg-nepal-orange text-white px-3 py-1 rounded-full text-sm">
                  Search: "{searchTerm}"
                </span>
              )}
              {selectedRegions.map((region) => (
                <span key={region} className="bg-nepal-primary text-white px-3 py-1 rounded-full text-sm">
                  {region}
                </span>
              ))}
              <button
